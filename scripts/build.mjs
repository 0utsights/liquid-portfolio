...
const ext = (label, href) => `<a class="text-link" href="${href}" target="_blank" rel="noopener noreferrer">${label} ${arrow}</a>`;
...
const projectRows = projects.map(p=>`<a class="project-row" href="/work/${p.slug}/"><span class="index">${p.number}</span><div><span class="meta">${p.category}</span><h3>${p.title}</h3><p>${p.summary}</p></div><span class="row-arrow" aria-hidden="true">↗</span></a>`).join('');
const ossRows = contributions.map(c=>`<article class="oss-row"><div><span class="meta">${c.tech}</span><h3>${c.name}</h3><a class="pr-link" href="https://github.com/${c.repo}/pull/${c.number}" target="_blank" rel="noopener noreferrer">Merged PR #${c.number} ${arrow}</a></div><div><p>${c.detail}</p></div></article>`).join('');

const home = `<section class="hero"><div class="hero-copy"><p class="eyebrow">SOFTWARE ENGINEERING</p><h1>John Surles.</h1><p class="hero-description">Computer Science at Virginia Tech.<br>Expected graduation: May 2028.</p><ul class="hero-facts"><li><a href="/work/#experience"><strong>Procentrix</strong> — Software Engineering Intern, 2026 ↗</a></li><li><a href="/research/"><strong>VTURCS</strong> — Undergraduate Researcher & Group Lead ↗</a></li><li><a href="/work/#open-source"><strong>Open source</strong> — Microsoft PowerToys, NASA-AMMOS & more ↗</a></li></ul><div class="hero-actions"><a class="button" href="/John-Surles-Resume.pdf">Résumé <span aria-hidden="true">↗</span></a><a class="quiet-link" href="/work/">Work & contributions <span aria-hidden="true">↗</span></a></div></div></section>
...
const work = `<section class="page-intro"><h1>Work.</h1><nav class="section-nav" aria-label="Work sections"><a href="#projects">Projects</a><a href="#experience">Internship</a><a href="#open-source">Open source</a></nav></section>
<section class="content-section" id="projects">${sectionHead('01','Projects')}<div class="project-list">${projectRows}</div></section>
<section class="content-section" id="experience">${sectionHead('02','Internship')}<article class="experience"><div><span class="meta">JUNE — AUGUST 2026</span><h3>Procentrix</h3><p>Software Engineering Intern</p></div><div class="experience-copy"><ul class="detail-list"><li>Built a reusable Node.js / TypeScript backend with SQL Server, including authenticated REST APIs and input validation.</li><li>Developed a React financial risk assessment interface with Dataverse, Fluent UI, and reusable master-detail components.</li><li>Published shared developer tools and guidance for React scaffolding and Dataverse authentication.</li></ul></div></article></section>
<section class="content-section" id="open-source">${sectionHead('03','Open source','<span class="meta">Merged contributions</span>')}<div class="oss-list">${ossRows}</div></section>`;
...
<section class="content-section contact-section" id="contact"><h2>Contact.</h2><a class="contact-email" href="mailto:surlesjohn@outlook.com">surlesjohn@outlook.com ${arrow}</a></section>`;

function projectPage(p) {
 const next=projects[(projects.indexOf(p)+1)%projects.length];
 return `<section class="page-intro project-intro"><a class="back-link" href="/work/">← All work</a><p class="eyebrow">${p.category.toUpperCase()}</p><h1>${p.title}</h1><p>${p.summary}</p><div class="inline-links">${p.links.map(([label,href])=>ext(label,href)).join('')}</div></section><section class="project-facts"><div><span class="meta">ROLE</span><span>Developer</span></div><div><span class="meta">TIMELINE</span><span>${p.date}</span></div><div><span class="meta">STACK</span><span>${p.stack.join(' · ')}</span></div></section><section class="content-section">${sectionHead('01','Contributions')}<ul class="detail-list compact-contributions">${p.contribution.map(item=>`<li>${item}</li>`).join('')}</ul></section><a class="next-project" href="/work/${next.slug}/"><span class="meta">NEXT PROJECT</span><span>${next.title} ${arrow}</span></a>`;
}
...
<body><a class="skip-link" href="#main">Skip to content</a><div class="ambient" aria-hidden="true"><canvas id="sculpture"></canvas><div class="ambient-shade"></div></div><header class="site-header"><a class="wordmark" href="/" aria-label="John Surles, home">john surles<span>/</span></a><nav class="main-nav" aria-label="Main navigation">${nav}</nav><a class="header-contact" href="mailto:surlesjohn@outlook.com">Email ${arrow}</a></header><main id="main" tabindex="-1">${route.body}</main><footer class="site-footer"><div class="footer-top"><a class="wordmark" href="/">john surles<span>/</span></a><div class="footer-links">${ext('GitHub','https://github.com/0utsights')}${ext('LinkedIn','https://www.linkedin.com/in/john-surles-650a16389/')}<a class="text-link" href="mailto:surlesjohn@outlook.com">Email ${arrow}</a><a class="text-link" href="/John-Surles-Resume.pdf">Résumé ${arrow}</a></div></div><div class="footer-bottom"><span>© 2026 John Surles · Blacksburg, Virginia</span><div class="preferences" hidden><button id="motion-toggle" type="button" aria-pressed="false">Pause motion</button><span aria-hidden="true">/</span><button id="sound-toggle" type="button" aria-pressed="false">Sound off</button></div></div></footer><noscript><style>.ambient{background:radial-gradient(ellipse at 85% 25%,#b1b4ac55,transparent 60%)}.sculpture-caption{display:none}</style></noscript></body></html>`;
}

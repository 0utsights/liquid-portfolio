...
const notes = (p, expanded = false) => {
 const note = engineeringNotes[p.slug];
 return note ? '<details class="engineering-note"' + (expanded ? ' open' : '') + '><summary>Engineering notes <span aria-hidden="true">+</span></summary><div class="note-body"><h4>Problem</h4><p>' + note.problem + '</p><h4>Implementation</h4><p>' + note.decision + '</p><h4>System behavior</h4><p>' + note.detail + '</p>' + ext(...note.proof) + '</div></details>' : '';
};
...
const ossRows = contributions.map(c=>`<article class="oss-row"><div><span class="meta">${c.tech}</span><h3>${c.name}</h3><a class="pr-link" href="https://github.com/${c.repo}/pull/${c.number}" target="_blank" rel="noopener noreferrer">Merged PR #${c.number}</a></div><div>${c.breakdown ? `<dl class="contribution-breakdown">${c.breakdown.map(([label, description])=>`<div><dt>${label}</dt><dd>${description}</dd></div>`).join('')}</dl>` : `<p>${c.detail}</p>`}</div></article>`).join('');

const home = `<section class="hero" aria-labelledby="intro-name">
 <div class="hero-copy"><p class="eyebrow">Software engineering / Backend systems</p><h1 id="intro-name">John <em>Surles</em><span class="name-period" aria-hidden="true">.</span></h1><p class="hero-statement">I build backend applications<br> and tools for operating them.</p><p class="hero-focus">Authenticated APIs, relational data, and Linux tooling. Seeking software engineering internships with a backend and systems focus.</p><div class="hero-actions"><a class="button" href="/John-Surles-Resume.pdf">View resume <span class="file-label">PDF</span></a>${ext('GitHub','https://github.com/0utsights')}${ext('LinkedIn','https://www.linkedin.com/in/john-surles-650a16389/')}</div></div>
...
<section class="content-section home-selection" id="selected">${sectionHead('Selected projects', '<span class="meta">Persistence, observability, and event-driven state</span>')}<div class="project-list">${projectRows}</div></section>
<section class="content-section" aria-labelledby="repositories-title"><div class="section-heading"><h2 id="repositories-title">More on GitHub</h2>${ext('All repositories', 'https://github.com/0utsights?tab=repositories')}</div><div class="repository-grid"><article><h3>Aeyori / KarutaBot</h3><p>Python OCR and workflow coordination, with configurable routines, cooldown handling, and activity logs. An earlier hosted version included a FastAPI backend with authentication and license-key management.</p><div class="inline-links"><a class="text-link" href="/work/aeyori/">Project details</a>${ext('Code on GitHub', 'https://github.com/0utsights/KarutaBot')}</div></article><article><h3>Canvas Anki AI</h3><p>Python add-on in active development: discovers Canvas course material and extracts source-aware content from documents for draft flashcards.</p>${ext('Canvas Anki AI repository', 'https://github.com/0utsights/canvas-anki-ai')}</article></div></section>`;

const work = `<section class="page-intro"><h1>Work</h1><p>Backend API development, debugging in established codebases, and contributions backed by regression tests.</p><nav class="section-nav" aria-label="Work sections"><a href="#experience">Experience</a><a href="#open-source">Open source</a><a id="projects" href="/#selected">Projects</a></nav></section>
...
const about = `<section class="page-intro"><h1>About</h1><p>I’m a Computer Science student at Virginia Tech, interested in backend software engineering: how APIs define behavior, how applications persist data, and how operators understand running systems.</p><p>At Procentrix, I built authenticated REST APIs with TypeScript and SQL Server. My independent work spans a deployed PostgreSQL marketplace, Go tooling for Linux servers, and event-driven Java software.</p><p>I’m seeking software engineering internships where I can contribute to backend services and systems while learning from experienced engineers.</p></section>
...
<section class="content-section">${sectionHead('Skills')}<div class="skill-row"><h3>Languages</h3><p>TypeScript, Go, Python, Java, SQL, C++</p></div><div class="skill-row"><h3>Backend & data</h3><p>Node.js, FastAPI, REST APIs, PostgreSQL, SQL Server, Prisma, authentication, input validation</p></div><div class="skill-row"><h3>Application development</h3><p>React, Next.js, Dataverse, Fabric API</p></div><div class="skill-row"><h3>Infrastructure</h3><p>Linux, Docker, SSH, Terraform, GitHub Actions, Git, AWS, Oracle Cloud</p></div></section>`;

function projectPage(p) {
 const next=featuredProjects[(featuredProjects.indexOf(p)+1)%featuredProjects.length];
 return `<section class="page-intro project-intro"><a class="back-link" href="/#selected">All projects</a><p class="eyebrow">${p.category}</p><h1>${displayTitle(p)}</h1><p>${p.summary}</p><div class="inline-links">${p.links.map(([label,href])=>ext(label,href)).join('')}</div>${p.sourceNote ? `<p class="source-note">${p.sourceNote}</p>` : ''}</section><section class="project-facts"><div><span class="meta">Timeline</span><span>${p.date}</span></div><div><span class="meta">Technologies</span>${tags(p.stack)}</div></section><section class="content-section">${sectionHead('Contributions')}<div class="project-detail-grid"><ul class="detail-list compact-contributions">${p.contribution.map(item=>`<li>${item}</li>`).join('')}</ul>${projectPreview(p)}</div></section>${engineeringNotes[p.slug] ? `<section class="content-section technical-section">${sectionHead('Inside the project')}${flowDiagram(p)}${notes(p, true)}</section>` : ''}<a class="next-project" href="/work/${next.slug}/"><span class="meta">Next project</span><span>${displayTitle(next)}</span></a>`;
}

const routes=[
 {path:'/',key:'home',title:'John Surles — Backend Software Engineering',description:'Virginia Tech Computer Science student focused on backend software engineering: authenticated APIs, PostgreSQL applications, Linux tooling, and open-source contributions.',body:home},
 ...
 {path:'/about/',key:'about',title:'About — John Surles',description:'Virginia Tech Computer Science student seeking backend and systems software engineering internships. Former Procentrix intern and open-source contributor.',body:about},

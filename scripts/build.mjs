...
const projectRows = projects.map(p=>`<article class="project-row"><div class="project-summary"><h3><a href="/work/${p.slug}/">${displayTitle(p)}</a></h3><p>${p.summary}</p></div><div class="project-support">${tags(p.stack)}<div class="project-evidence"><a class="text-link" href="/work/${p.slug}/" aria-label="View ${p.title} project">View project</a>${p.links.slice(0,1).map(([label,href])=>ext(label,href)).join('')}</div></div></article>`).join('');
...
<section class="home-highlights" aria-label="Experience, open source, and research">
 ...
 <article><h2>Research</h2><h3>Virginia Tech · VTURCS</h3><p class="highlight-role">Undergraduate Researcher & Group Lead<span class="meta">September 2026–Present</span></p><p>I develop semantic search and lead one of three student groups under faculty supervision.</p><div class="highlight-links"><a class="text-link" href="/research/">Research details</a></div></article>
</section>
<section class="content-section home-selection" id="selected">${sectionHead('Projects','<a class="text-link" href="/work/">All work & contributions</a>')}<div class="project-list">${projectRows}</div></section>`;
...
<section class="content-section" id="projects">${sectionHead('Projects')}<div class="project-list">${projectRows}</div></section>`;
...
function projectPage(p) {
 ...
 return `<section class="page-intro project-intro"><a class="back-link" href="/work/">All work</a><p class="eyebrow">${p.category}</p><h1>${displayTitle(p)}</h1><p>${p.summary}</p><div class="inline-links">${p.links.map(([label,href])=>ext(label,href)).join('')}</div>${p.sourceNote ? `<p class="source-note">${p.sourceNote}</p>` : ''}</section><section class="project-facts"><div><span class="meta">Role</span><span>Developer</span></div><div><span class="meta">Timeline</span><span>${p.date}</span></div><div><span class="meta">Technologies</span>${tags(p.stack)}</div></section><section class="content-section">${sectionHead('Contributions')}<div class="project-detail-grid"><ul class="detail-list compact-contributions">${p.contribution.map(item=>`<li>${item}</li>`).join('')}</ul>${projectPreview(p)}</div></section><a class="next-project" href="/work/${next.slug}/"><span class="meta">Next project</span><span>${displayTitle(next)}</span></a>`;
}
...
<body><a class="skip-link" href="#main">Skip to content</a><div class="ambient" aria-hidden="true"><canvas id="sculpture"></canvas><div class="ambient-shade"></div></div><header class="site-header"><a class="wordmark" href="/" aria-label="John Surles, home">John Surles</a><nav class="main-nav" aria-label="Main navigation">${nav}</nav><div class="header-actions"><a class="text-link" href="mailto:surlesjohn@outlook.com">Email</a><a class="text-link" href="/John-Surles-Resume.pdf" aria-label="Resume (PDF)">Resume</a></div></header><main id="main" tabindex="-1">${route.body}</main><footer class="site-footer"><div class="footer-top"><a class="wordmark" href="/">John Surles</a><div class="footer-links">${ext('GitHub','https://github.com/0utsights')}${ext('LinkedIn','https://www.linkedin.com/in/john-surles-650a16389/')}<a class="text-link" href="mailto:surlesjohn@outlook.com">Email</a><a class="text-link" href="/John-Surles-Resume.pdf">Resume</a></div></div><div class="footer-bottom"><span>© 2026 John Surles · Blacksburg, Virginia</span><div class="preferences" hidden><button id="motion-toggle" type="button" aria-pressed="false">Pause motion</button><button id="sound-toggle" type="button" aria-pressed="false">Sound off</button></div></div></footer><noscript><style>.ambient{background:radial-gradient(ellipse at 85% 25%,#b1b4ac55,transparent 60%)}.sculpture-caption{display:none}</style></noscript></body></html>`;
}
